...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "gestionRed-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name gestionRed-service apps/gestionRed-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app gestionRed-service:1.0-SNAPSHOT
    oc expose svc/gestionRed-service
else

	echo "Launching the application locally..."
	pattern="gestionRed-service"
	files=( $pattern )
	cd ${files[0]}
