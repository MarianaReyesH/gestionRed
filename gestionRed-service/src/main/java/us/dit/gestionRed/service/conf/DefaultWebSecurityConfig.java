...
public class DefaultWebSecurityConfig {
	...
	UserDetailsService userDetailsService(BCryptPasswordEncoder encoder) {
		...
		UserDetails gestionRed = User.withUsername("gestionRed").password(encoder.encode("gestionRed")).roles("kie-server").build();
		...
		return new InMemoryUserDetailsManager(wbadmin, user, kieserver, gestionRed);
	}
