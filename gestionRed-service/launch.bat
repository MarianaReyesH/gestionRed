...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "gestionRed-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name gestionRed-service apps/gestionRed-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app gestionRed-service:1.0-SNAPSHOT
		call oc expose svc/gestionRed-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd gestionRed-service
		cd target
